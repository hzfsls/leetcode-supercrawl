## [933.最近的请求次数 中文热门题解1](https://leetcode.cn/problems/number-of-recent-calls/solutions/100000/by-ac_oier-evqe)
...
```
* 时间复杂度：通常情况下：调用次数为 $m$，值域大小为 $n$，复杂度为 $O(\sqrt{n})$；本题：$O(M)$，其中 $M$ 分块数组大小
